...
import {getActuations} from '../../../services/services-lexon';
...
export class ConnectingEmailsStep3 extends React.Component {
  constructor() {
    ...
    this.renderDate = this._renderDate.bind(this);
    this.gridRef = null;
  ...
  async componentDidUpdate(prevProps, prevState) {
    ...
    if (
      ...
      try {
        ...
        const response = await getActuations(
          user.idUser,
          bbdd.bbdd,
          entity,
          search,
  ...
  _renderCheck(props) {
    const ix = props.idRelated + '_' + props.idActuation;
  ...
  _renderDate(props) {
    const m = moment(props.date).format('DD/MM/YYYY');
    return <span>{m}</span>;
  }
        ...
        <div className='step2-container'>
          <ol style={{ textAlign: 'center' }}>
            <li className='index-3'>
              <span>{i18n.t(`classification-calendar.step3.q1`)}</span>
            </li>
          </ol>
          <section className='section-border'>
            <p className='section-title'>
              {i18n.t(`classification-calendar.step1b.${entity}`)}
            </p>
