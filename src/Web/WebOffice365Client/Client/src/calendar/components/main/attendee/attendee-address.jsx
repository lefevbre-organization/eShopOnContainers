...
export class AttendeeAddress extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    return (
      <div className={`input-group containeratt`} onClick={() => this.fieldClick()}
        onDragOver={e => e.preventDefault()} onDrop={e => this.onDrop(e, id)}>
            <div className={'input-group-prepend'}><div className="input-group-text attendeetext">{i18n.t('schedule.attendees')}</div></div>
        {addresses.map((address, index) => (
          <div key={index} className={`mdc-chip`}
            draggable={true}
            onDragStart={event => AttendeeAddress.onAddressDragStart(event, id, address)}>
  ...
  onSuggestionSelected(event, {suggestionValue}) {
    ...
    //setTimeout(() => AttendeeAddress.clearValidation(this.inputRef.current.input));
  }
  ...
  onHeaderKeyDown(event) {
    const target = event.target;
    AttendeeAddress.clearValidation(target);
...
export default (AttendeeAddress);
