  ...
  async componentDidUpdate(prevProps) {
    ...
    if (!emailMessageResult.loading) {
      if (!emailMessageResult.failed) {
            ...
            for (var i = 0; i < attach.length; i++) {
              if (attach[i].contentBytes) {
                ...
                if (attach[i].isInline === false) {
                  ...
                  var AttachmentDiv = addAttachmentContainer(
                    attach[i].contentType
                  );
        ...
        if (
          this.props.match.path == '/:id([a-zA-Z0-9!@#$%^&+=_-]+)' &&
          this.props.match.url !== '/inbox'
        ) {
          if (
            (emailMessageResult.error.statusCode === 400 ||
              emailMessageResult.error.statusCode === 404) &&
