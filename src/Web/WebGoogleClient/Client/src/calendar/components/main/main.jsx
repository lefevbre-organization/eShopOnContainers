...
export class Main extends Component {
  constructor(props) {
    ...
    this.toasts = [
      {
        content: i18n.t('schedule.toast-processing'),
        cssClass: 'e-toast-black',
        icon: '',
      },
      {
        content: i18n.t('schedule.toast-process-complete'),
        cssClass: 'e-toast-black',
        icon: '',
      },
      {
        content: i18n.t('schedule.toast-process-error'),
  ...
  openCalendarView(args) {
    ...
    let calendarId = '';
    if (args != undefined) calendarId = args.currentTarget.id;
    this.setState({
      hidePromptDialog: true,
      calendarToEdit: calendarId,
    });
  }
  // Calednar View Dialog
  dialogClose(args) {
  ...
  toastPreventDuplicate(e) {
    ...
    for (let i = 0; i < toasts.length; i++) {
      ...
      if (
        toastTitle &&
        toastTitle.isEqualNode(toastEle.querySelector('.e-toast-title'))
      ) {
        return true;
      }
      if (
        !toastTitle &&
        toastMessage &&
        toastMessage.isEqualNode(toastEle.querySelector('.e-toast-message'))
      ) {
        return true;
      }
    }
      ...
      <div>
        <div className='image'>
          <img
            width='16'
            height='16'
            src={'assets/img/' + props.ImageName + '.png'}
          />{' '}
          {props.Subject}
        </div>
    ...
    if (items.length > 0) {
      for (let i = 0; i < items.length; i++) {
        ...
        if (event.status === 'cancelled') {
          ...
          var dateString =
            moment(dateStartTime).seconds(0).toISOString().split('.')[0] + 'Z';
          var ExcRecurenceDate = dateString.replace(/[:.-]/g, '');
          var ParentscheduleException = '';
          var coma = '';
          if (
            this.scheduleData.find((x) => x.Id == event.recurringEventId)
              .RecurrenceException != undefined
          ) {
            ParentscheduleException = this.scheduleData.find(
              (x) => x.Id == event.recurringEventId
            ).RecurrenceException;
            coma = ',';
          }
          this.scheduleData.find(
            (x) => x.Id == event.recurringEventId
          ).RecurrenceException =
            ParentscheduleException + coma + ExcRecurenceDate;
          continue;
        }
  ...
  LoadCalendarList(DisableloadSchedule) {
    ...
    listCalendarList()
      .then((result) => {
        ...
        this.resourceCalendarData.find(
          (x) => x.id == this.resourceCalendarData[0].id
        ).checked = true;
  ...
  buildEventoGoogle(values) {
    ...
    if (values.RecurrenceRule != undefined) {
      event.recurrence = ['RRULE:' + values.RecurrenceRule];
    }
  ...
  onPopupOpen(args) {
    ...
    if (args.type === 'Editor') {
      ...
      if (!args.element.querySelector('.custom-field-row')) {
        ...
        let containerEventType = createElement('div', {
        ...
        let drowDownList = new DropDownList({
          dataSource: [
            {
              text: i18n.t('schedule.profesional-event'),
              value: 'profesional-event',
            },
            {
              text: i18n.t('schedule.personal-event'),
          ...
          placeholder: i18n.t('schedule.eventtype'),
        });
        drowDownList.appendTo(inputEle);
        inputEle.setAttribute('name', 'EventType');

        // Adding attendees tag element
        let containerTab = createElement('div', {
          className: 'custom-field-container',
        });
    ...
    switch (args.requestType) {
        ...
        if (args.data.occurrence != undefined) {
          ...
          if (singleEventToRemove.length > 0) {
            eventItemToModify =
              args.data.parent.Id +
              '_' +
              singleEventToRemove[singleEventToRemove.length - 1];
          } else {
            eventItemToModify =
              args.data.parent.Id + '_' + singleEventToRemove[0];
          }
          this.deleteCalendarEventCRUD(
            args.data.parent.CalendarId,
            eventItemToModify,
            true
          );
      ...
      case 'eventRemoved':
        ...
        if (args.data[0].occurrence != undefined) {
          ...
          var dateString =
            moment(d).seconds(0).toISOString().split('.')[0] + 'Z';
          var ExcRecurenceDate = dateString.replace(/[:.-]/g, '');
                    ...
                    <div className='control-wrapper'>
                      <ScheduleComponent
                        ...
                        <ResourcesDirective>
                          ...
                          <ResourceDirective
                            field='CalendarId'
                            title={i18n.t('calendar-sidebar.mycalendars')}
                            name='Calendars'
                            allowMultiple={false}
                            dataSource={this.resourceCalendarData}
                            textField='summary'
                            idField='id'
                            colorField='backgroundColor'></ResourceDirective>
                        </ResourcesDirective>
