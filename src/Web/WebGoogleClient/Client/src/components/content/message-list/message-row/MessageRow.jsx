...
export class MessageItem extends Component {
  ...
  getMessage(evt) {
    if(this.props.data.labelIds.length > 0 && this.props.data.labelIds.includes('DRAFT')){
  ...
  render() {
    ...
    const unread =
      this.props.data.labelIds.indexOf('UNREAD') > -1
        ? ' font-weight-bold'
        : '';
    const selected = this.props.data.selected ? ' selected' : '';
    const subjectHeader = this.props.data.payload.headers.find(
      (el) => el.name.toUpperCase() === 'SUBJECT'
    );
    const subject = subjectHeader ? subjectHeader.value : '';
    const fromHeader = this.props.data.payload.headers.find(
      (el) => el.name.toUpperCase() === 'FROM'
    );
    const toHeader = this.props.data.payload.headers.find(
      (el) => el.name.toUpperCase() === 'TO'
    );
    let fromName = '';
        ...
        >
          ...
          <AttachmentDateFields
            ...
            hasAttachment={
              ...
              (this.props.data.payload.mimeType === 'multipart/alternative' &&
                JSON.stringify(this.props.data.payload.headers).indexOf(
                  'Apple-Mail'
                ) > -1)
            }
          />
