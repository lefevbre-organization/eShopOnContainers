window.REACT_APP_ENVIRONMENT = 'LOCAL';
window.URL_UPDATE_DEFAULTACCOUNT =
  'https://lexbox-test-apigwacc.lefebvre.es/api/v1/accounts/emailuseraccount/updatedefaultaccount';
window.URL_RESET_DEFAULTACCOUNT =
  'https://lexbox-test-apigwacc.lefebvre.es/api/v1/accounts/emailuseraccount/resetdefaultaccountbyuser';
window.URL_DELETE_ACCOUNT =
  'https://lexbox-test-apigwacc.lefebvre.es/api/v1/accounts/emailuseraccount/deleteaccountbyuserandprovider';
window.URL_SELECT_ACCOUNT = 'http://localhost:3010';
window.URL_GET_ACCOUNTS =
  'https://lexbox-test-apigwacc.lefebvre.es/api/v2/accounts/usermail';

// window.URL_UPDATE_DEFAULTACCOUNT =
//     'http://localhost:8083/api/v1/accounts/emailuseraccount/updatedefaultaccount';
// window.URL_RESET_DEFAULTACCOUNT =
//     'http://localhost:8083/api/v1/accounts/emailuseraccount/resetdefaultaccountbyuser';
// window.URL_DELETE_ACCOUNT =
//     'http://localhost:8083/api/v1/accounts/emailuseraccount/deleteaccountbyuserandprovider';
// window.URL_SELECT_ACCOUNT = 'http://localhost:3010';
// window.URL_GET_ACCOUNTS =
//     'http://localhost:8083/api/v2/accounts/usermail';

window.URL_MF_GOOGLE = 'http://localhost:3001/user';
window.URL_MF_OUTLOOK = 'http://localhost:3002/user';
window.URL_MF_IMAP = 'http://localhost:9000/user';
window.URL_MF_LEXON = 'https://lexbox-test-weblexon.lefebvre.es/static/js/main.js';
window.URL_MF_LEXON_BASE = 'https://lexbox-test-weblexon.lefebvre.es';
window.URL_MF_CENTINELA = 'http://localhost:3005/static/js/main.js';
window.URL_MF_CENTINELA_BASE = 'https://lexbox-test-webcentinela.lefebvre.es';

window.URL_MF_LEXON_BASE = 'http://localhost:3004';
// window.URL_MF_LEXON =
//   'https://lexbox-test-weblexon.lefebvre.es/static/js/main.js';
window.API_GATEWAY = 'https://lexbox-test-apigwlex.lefebvre.es';
window.API_CENTINELA_GATEWAY =
  'https://lexbox-test-apigwcen.lefebvre.es/api/v1/cen';
window.URL_MF_DATABASE = 'http://localhost:3006/static/js/main.js';
window.URL_MF_DATABASE_BASE = 'http://localhost:3006';

window.RELEASE = 'RC DEV';
window.SHOW_EXPERIMENTAL = '1';
