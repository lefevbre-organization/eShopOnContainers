...
namespace Minihub.API.Infrastructure
{
    //mirar https://www.mongodb.com/blog/post/working-with-mongodb-transactions-with-c-and-the-net-framework
    public class MinihubContext : IMongoDbContext, IIntegrationEventLogContextMongoDB
    {
        ...
        private static void ClassMapping()
        {
            ...
            if (!BsonClassMap.IsClassMapRegistered(typeof(MinihubUser))) { BsonClassMap.RegisterClassMap<MinihubUser>(); }
        }
        ...
        public IMongoCollection<MinihubUser> MinihubUsers
        {
            get { return Database.GetCollection<MinihubUser>(_settings.Value.Collection); }
        }

        public IMongoCollection<MinihubUser> MiniUsersTransaction(IClientSessionHandle session)
        {
            return session.Client.GetDatabase(_settings.Value.Database).GetCollection<MinihubUser>(_settings.Value.Collection);
        }
