using Minihub.API.IntegrationsEvents.Events;
...
namespace Minihub.API.Infrastructure.Repositories
{
    public class MinihubRepository : BaseClass<MinihubRepository>, IMinihubRepository
    {
        private readonly MinihubContext _context;
        private readonly IOptions<MinihubSettings> _settings;

        public MinihubRepository(
              IOptions<MinihubSettings> settings
            , IEventBus eventBus
            , ILogger<MinihubRepository> logger

            ) : base(logger)
        {
            _settings = settings;
            _context = new MinihubContext(settings, eventBus);
        }
        ...
        //    var fields = Builders<MinihubUser>.Projection
        ...
        //    try
        //    {
        //        var user = await _context.MinihubUser
        //                    .Find(filter)
