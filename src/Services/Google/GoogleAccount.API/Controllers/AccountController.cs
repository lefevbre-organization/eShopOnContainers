...
namespace Lefebvre.eLefebvreOnContainers.Services.Google.Account.API.Controllers
{
    ...
    {
        private IGoogleAccountService _service;
        private readonly IOptions<GoogleDriveSettings> _settings;
        internal readonly ILogger<AccountController> _log;

        public AccountController(
          IGoogleAccountService service
        ...
        [HttpGet("{idNavision}")]
        [ProducesResponseType(typeof(Result<UserGoogleAccount>), (int)HttpStatusCode.OK)]
        [ProducesResponseType(typeof(Result<UserGoogleAccount>), (int)HttpStatusCode.BadRequest)]
            ...
            short idApp = 1)
        {
            ...
            Result<UserGoogleAccount> result = await _service.GetUserAsync(idNavision, idApp);
        ...
        public async Task<IActionResult> PostUserAsync(
                [FromBody] UserGoogleAccount user
            )
        {
            if (string.IsNullOrEmpty(user.idNavision))
                return BadRequest("Must be a valid idUserNavision");

            Result<UserGoogleAccount> result = await _service.PostUserAsync(user);
