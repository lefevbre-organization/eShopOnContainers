Param(
    ...
    [parameter(Mandatory=$false)][string[]]$charts=(
        "calendar-api", "conference-api", "lexon-api", "account-api", "centinela-api", "userutils-api", "signature-api",  # "database-api",
    ...
    [parameter(Mandatory=$false)][string]$imageTag="linux-dev-42",
...
if ($deployInfrastructure) {
    ...
    foreach ($infra in $infras) {
        ...
        helm upgrade --install "$appName-$infra" --values app-lef.yaml --values inf-lef.yaml --values $ingressValuesFile --set app.name=$appName --set inf.k8s.dns=$dns --set "ingress.hosts={$dns}"  $infra     
    }
}
else {
...
if ($deployCharts) {
    ...
    foreach ($chart in $charts) {
        ...
        if ($useCustomRegistry) {
            ...
            helm upgrade --install "$appName-$infra" --set inf.registry.server=$registry --set inf.registry.login=$dockerUser --set inf.registry.pwd=$dockerPassword --set inf.registry.secretName=elef-docker-secret --values app-lef.yaml --values inf-lef.yaml --values $ingressValuesFile --set app.name=$appName --set inf.k8s.dns=$dns --set "ingress.hosts={$dns}" --set image.tag=$imageTag --set image.pullPolicy=$pullPolicy  $chart 
        }
        else {
            if ($chart -ne "eshop-common")  {       # eshop-common is ignored when no secret must be deployed
                ...
                helm upgrade --install "$appName-$infra" --values app-lef.yaml --values inf-lef.yaml --values $ingressValuesFile --set app.name=$appName --set inf.k8s.dns=$dns  --set "ingress.hosts={$dns}" --set image.tag=$imageTag --set image.pullPolicy=$pullPolicy $chart 
            }
        }
    }
}
