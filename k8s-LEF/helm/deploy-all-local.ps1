Param(
    ...
    [parameter(Mandatory=$false)][string]$appName="elefebvre-dev",
...
if ($clean) {
    ...
    helm delete --purge $(helm ls -q elefebvre-dev) 
...
$charts = ( "apigwlex", "lexon-api", "lexonmysql-api", "apigwacc", "account-api" , "webgoogle", "webgraph", "weblexon", "webportal", "webimap", "webimapserver", "webstatus")

if ($deployInfrastructure) {
    foreach ($infra in $infras) {
        Write-Host "Installing infrastructure: $infra" -ForegroundColor Green
        helm install --namespace dev --values app.yaml --values inf.yaml --values $ingressValuesFile --set app.name=$appName --set inf.k8s.dns=$dns --set "ingress.hosts={$dns}" --name="$appName-$infra" $infra     
    }
}
else {
    ...
    foreach ($chart in $charts) {
        ...
        if ($useCustomRegistry) {
            ...
            if ($chart -ne "eshop-common")  {       # eshop-common is ignored when no secret must be deployed
              helm install --namespace dev --values app.yaml --values inf.yaml --values $ingressValuesFile --set app.name=$appName --set inf.k8s.dns=$dns  --set "ingress.hosts={$dns}" --set image.tag=$imageTag --set image.pullPolicy=$pullPolicy --name="$appName-$chart" $chart 
            }
        }
    }
}
