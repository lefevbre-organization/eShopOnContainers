Param(
    ...
    [parameter(Mandatory=$false)][string]$appName="elefebvre-pre",
...
if ($clean) {
    ...
    helm delete --purge $(helm ls -q elefebvre-pre) 
...
if ($deployInfrastructure) {
    foreach ($infra in $infras) {
        ...
        helm install --namespace pre --values app.yaml --values inf.yaml --values $ingressValuesFile --set app.name=$appName --set inf.k8s.dns=$dns --set "ingress.hosts={$dns}" --name="$appName-$infra" $infra     
    }
}
else {
    ...
    foreach ($chart in $charts) {
        ...
        if ($useCustomRegistry) {
            ...
            if ($chart -ne "eshop-common")  {       # eshop-common is ignored when no secret must be deployed
              helm install --namespace pre --values app.yaml --values inf.yaml --values $ingressValuesFile --set app.name=$appName --set inf.k8s.dns=$dns  --set "ingress.hosts={$dns}" --set image.tag=$imageTag --set image.pullPolicy=$pullPolicy --name="$appName-$chart" $chart 
            }
        }
    }
}
