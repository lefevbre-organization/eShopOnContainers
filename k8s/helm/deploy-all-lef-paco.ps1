Param(
    ...
    [parameter(Mandatory=$false)][string]$appYalmFile="app-lef.yaml",
    [parameter(Mandatory=$false)][string]$infYalmFile="inf-lef.yaml",
    # [parameter(Mandatory=$false)][string]$imageTag="latest",
    [parameter(Mandatory=$false)][string]$imageTag="linux-dev",
    [parameter(Mandatory=$false)][bool]$useLocalk8s=$true,
    [parameter(Mandatory=$false)][bool]$useLocalImages=$true
    )
...
$infras = ("sql-data", "nosql-data", "rabbitmq", "keystore-data")
# $infras = ("sql-data", "nosql-data", "rabbitmq", "keystore-data", "basket-data")
$charts = ("apigwlex", "lexon-api", "lexon-mysql-api", "apigwacc", "account-api", "webgoogle", "webgraph", "weblexon", "webportal" )
...
Write-Host "file app: $appYalmFile file inf: $infYalmFile file ingress: $ingressValuesFile " -ForegroundColor DarkCyan
#"apigwlex", "lexon-api", "lexon-mysql-api", "apigwacc", "account-api", "webgoogle", "webgraph", "weblexon", "webportal"
if ($deployInfrastructure) {
    foreach ($infra in $infras) {
        helm lint $infra/
        Write-Host "Installing infrastructure: $infra" -ForegroundColor DarkCyan
        Write-Host "app name: $appName - inf.k8s.dns changue: $dns - ingress.hosts changue: $dns  - name :$appName-$infra" -ForegroundColor DarkCyan
        ...
        helm install --debug --values $appYalmFile --values $infYalmFile --values $ingressValuesFile --set app.name=$appName --set inf.k8s.dns=$dns --set "ingress.hosts={$dns}" --name="$appName-$infra" $infra     
    }
}
else {
    ...
    foreach ($chart in $charts) {
        ...
        if ($useCustomRegistry) {
            ...
            if ($chart -ne "eshop-common")  {       # eshop-common is ignored when no secret must be deployed
              ...
              Write-Host "Installing chart $chart - app name: $appName - inf.k8s.dns changue: $dns - ingress.hosts changue: $dns  - name :$appName-$infra" -ForegroundColor DarkGreen
              Write-Host "image tag: $imageTag pull policy: $pullPolicy name: $appName-$chart " -ForegroundColor DarkGreen
              helm install --debug --values $appYalmFile --values $infYalmFile --values $ingressValuesFile --set app.name=$appName --set inf.k8s.dns=$dns  --set "ingress.hosts={$dns}" --set image.tag=$imageTag --set image.pullPolicy=$pullPolicy --name="$appName-$chart" $chart 
            }
        }
    }
}
